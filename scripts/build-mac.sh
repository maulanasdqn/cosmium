...
if should_run "fetch"; then
  ...
  if [[ -d "$SRC_DIR/chrome" ]]; then
    ...
    info "Initializing git repo in src/..."
    cd "$SRC_DIR"
    git init -q
    git add -A
...
for p in sorted(paths):
    ...
    if os.path.isdir(local) and not os.path.isdir(os.path.join(local, '.git')):
        ...
        env = {**os.environ,
               'GIT_AUTHOR_NAME': 'cosmium', 'GIT_AUTHOR_EMAIL': 'c@b',
               'GIT_COMMITTER_NAME': 'cosmium', 'GIT_COMMITTER_EMAIL': 'c@b'}
...
if should_run "build"; then
  ...
  if [[ -d "$BUILD_OUT/Chromium.app" ]]; then
    SIZE=$(du -sh "$BUILD_OUT/Chromium.app" | cut -f1)
    ok "App: ${BUILD_OUT}/Chromium.app (${SIZE})"
  fi
fi

echo ""
info "🎉 Cosmium ${VERSION} macOS ARM64 build finished!"
